...
export function ParallaxImage({src,alt,priority=false}:{src:string;alt:string;priority?:boolean}){
 ...
 return <div ref={wrap} className="parallax-image"><div ref={img} className="parallax-image-inner"><Image src={src} alt={alt} fill sizes="(max-width: 760px) 100vw, 50vw" priority={priority} onLoad={(event)=>event.currentTarget.classList.add("is-loaded")}/></div></div>
}
